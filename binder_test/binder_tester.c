...
static int id;
static unsigned long data_SZ = 32;
...
void acsiidump(const void *buf, unsigned long size)
{
	printf("size = %lu\n",size);
	int col = 0, off = 0;
	unsigned char *p = (unsigned char *)buf;
...
inline void INST_ENTRY_COPY(inst_buf_t *inst, char *label, inst_entry_t *copy)
{
	if (inst->next_entry < inst->max_entries) {
		...
		//hexdump(inst,inst->max_entries * sizeof(inst_buf_t));
		//acsiidump(inst,4 * sizeof(inst_entry_t));
	}
}
...
void children_reaper(int sig)
{
	...
	printf("server(%d): ioctl read: %u\n",getpid(), ioctl_read);
	printf("server(%d): ioctl write: %u\n",getpid(), ioctl_write);
	printf("server(%d): ioctl buffer: %u\n",getpid(), ioctl_buffer);
	exit(0);
}
