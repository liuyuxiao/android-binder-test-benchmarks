...
typedef union {
    struct timeval tv;//record time
    char label[8];//first used as lable to printout,if inst_buf_t->seq=0
} inst_entry_t;

typedef struct {
    uint32_t magic;
    uint32_t seq;//counts of turn around
...
wait_reply:
    ...
    if (time_ref > 0 && iterations > 0) {
        ...
        if (!buf)
            return 1;
        if(time_ref <2) { //relative to begin
            for (n = 0; n < m; n++) {
                pos += sprintf(buf + pos, "\t%s: %lld.%02lldus  \tpercent:%.02f%%\n", labels[n],
                               (total_usecs[n] / iterations),
                               (total_usecs[n] % iterations) * 100 / iterations,100.0*(total_usecs[n]-(n>0?total_usecs[n-1]:0))/total_usecs[m-1]);
            }
        } else { //relative to pre
            unsigned long long  totalus = 0;
            for (n = 0; n < m; n++) {
                totalus += total_usecs[n];
            }
            for (n = 0; n < m; n++) {
                pos += sprintf(buf + pos, "\t%s: %lld.%02lldus  \tpercent:%.02f%%\n", labels[n],
                               (total_usecs[n] / iterations),
                               (total_usecs[n] % iterations) * 100 / iterations,100.0*total_usecs[n]/totalus);
